...
class NetworkTrainer(object):
    def __init__(self, deterministic=True, network=None, TARGET=None, model_VER=None, fold_SEED=None, select_FOLD=None, CUDA_device=None):
    ...
    def print_log_file(self, text, f):
        with open(f"{self.output_folder}/logs_{self.network}_{self.TARGET}.txt", "a+") as f:
            print(text, file=f)
        print(text)
    ...
    def run_training(self):
        ...
        f = open(os.path.join(self.output_folder, f"logs_{self.network}_{self.TARGET}.txt"), "w+")
        ...
        for self.fold, (train_idx, val_idx) in enumerate(kfold.split(test_csv)):
            ...
            if self.fold not in self.select_FOLD:
                ...
                train_data = MI_Dataset(dataframe=train_csv, mode="train", image_select=["pet", "rtct", "rtdose", "rtst_data"], target=self.TARGET)
                valid_data = MI_Dataset(dataframe=valid_csv, mode="val", image_select=["pet", "rtct", "rtdose", "rtst_data"], target=self.TARGET)
                ...
                valid_loader = torch.utils.data.DataLoader(valid_data, batch_size=self.batch_size, sampler=sampler_val, pin_memory=True)
                ...
                self.criterion = torch.nn.SmoothL1Loss(beta=13.0).to(self.device)
